<?php class Penduduk_model extends CI_Model {
	...
	{
		$select_sql = "SELECT DISTINCT u.id, u.nik, u.tanggallahir, u.tempatlahir, u.foto, u.status, u.status_dasar, u.id_kk, u.nama, u.nama_ayah, u.nama_ibu, a.dusun, a.rw, a.rt, d.alamat, d.no_kk AS no_kk, u.kk_level,
	...
	public function insert()
	{
		...
		if ($outp) {
			...
			// Auto generate mandiri user
			$nik = $this->db->select('nik')->where('id', $idku)
			->get('tweb_penduduk')->row()->nik;

			$hash_pin = hash_pin(substr($nik,-6));
			$data_mandiri['pin'] = $hash_pin;
			$data_mandiri['id_pend'] = $idku;
			$data_mandiri['tanggal_buat'] = date("Y-m-d H:i:s");
			$outp = $this->db->insert('tweb_penduduk_mandiri', $data_mandiri);
		}
    ...
    $data = $_POST;
    $data['kelahiran_anak_ke'] = (int)$data['kelahiran_anak_ke'] > 0 ? (int)$data['kelahiran_anak_ke'] : 1;
...
if (!empty($foto = $this->input->post('foto')))
	{
		$foto = base64_decode(explode('base64,', $foto)[1]);
		$k =  uniqid();
		file_put_contents($file = FCPATH.'desa/upload/user_pict/'.$k.'.png', $foto);
		$dimensi = array("width"=>200, "height"=>267);
		resizeImage($file, 'image/png', $dimensi, $nama_file = FCPATH.'desa/upload/user_pict/kecil_'.$k.'.png');
  if($old_foto!="") {
    // Hapus old_foto
	...
	public function get_penduduk_raw($id = 0)
	{
