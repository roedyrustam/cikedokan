<?php class Mandiri_model extends CI_Model {
	...
	public function insert_generator($data_insert)
	{
		$hash_pin = hash_pin($data_insert['pin']);
		$data['pin'] = $hash_pin;
		$data['id_pend'] = $data_insert['id'];
		$data['tanggal_buat'] = date("Y-m-d H:i:s");

		return $this->db->insert('tweb_penduduk_mandiri', $data);
	}
	...
	{
		$sql = "SELECT id AS id_penduduk, nik AS id, nik, nama FROM tweb_penduduk WHERE status = 1 AND nik <> '' AND nik <> 0";
		$query = $this->db->query($sql);
		$data = $query->result_array();
