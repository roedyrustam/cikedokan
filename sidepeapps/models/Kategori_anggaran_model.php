...
class Kategori_anggaran_model extends CI_Model {
	...
	public function autocomplete()
	{
		...
		where('parrent', 0)->
		order_by('kategori')->
		get('apbdes_kategori')->result_array();
	...
	{
		$sql = " FROM apbdes_kategori k WHERE parrent = 0";
		$sql .= $this->search_sql();
		$sql .= $this->filter_sql();
		return $sql;
	}
	...
	{
		$data = $_POST;
		$data['kategori_slug'] = url_title( $data['kategori'], '-', TRUE );
		$data['enabled'] = 1;
		$data['urut'] = $this->urut_max() + 1;
		$outp = $this->db->insert('apbdes_kategori', $data);
	...
	public function update($id=0)
	{
		...
		$data['kategori_slug'] = url_title( $data['kategori'], '-', TRUE );
		$this->db->where('id',$id);
		$outp = $this->db->update('apbdes_kategori', $data);
		if ($outp) $_SESSION['success'] = 1;
		else $_SESSION['success'] = -1;
	}
	...
	{
		$sql = "DELETE FROM apbdes_kategori WHERE id = ?";
	...
	{
		$sql = "SELECT * FROM apbdes_kategori WHERE parrent = ? ORDER BY urut";
	...
	public function list_link()
	{
		$sql = "SELECT a.*
		FROM artikel a
		INNER JOIN apbdes_kategori k ON a.id_kategori = k.id";
	...
	public function list_kategori($o="")
	{
		...
		$sql = "SELECT k.id,k.kategori AS kategori FROM apbdes_kategori k WHERE 1 ORDER BY $urut";
	...
	{
		$sql = "UPDATE apbdes_kategori SET enabled = ? WHERE id = ?";
	...
	private function urut_max($kategori='')
	{
		...
		$query = $this->db->get('apbdes_kategori');
	...
	public function urut($id, $arah, $kategori='')
	{
		...
		update('apbdes_kategori', array('urut' => $kategori1['urut']));
		$this->db->where('id', $kategori1['id'])->
		update('apbdes_kategori', array('urut' => $kategori2['urut']));
	}
