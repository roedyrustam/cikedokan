...
public function get_pamong_by_id($id='')
{
	$pamong = $this->db->select('*')->from('tweb_desa_pamong')->where('pamong_id', $id)->limit(1)->get()->row_array();
	return $pamong;
}
...
public function insert()
{
	...
	if (!empty($foto = $this->input->post('foto')))
	{
		...
		resizeImage($file, 'image/png', $dimensi, FCPATH.'desa/upload/user_pict/kecil_'.$k.'.png');
		$nama_file = $k.'.png';
	} else {
		$nama_file = '';
	}
...
private function siapkan_data($data)
{
	...
	$data['tupoksi'] = $this->input->post('tupoksi');
