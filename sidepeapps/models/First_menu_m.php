...
class First_menu_m extends CI_Model{
	...
	{
		$sql = "SELECT m.*, m.kategori as nama FROM kategori m WHERE m.parrent = 0 AND m.enabled = 1 AND m.kategori <> 'teks_berjalan' AND m.id != 1000 AND m.id != 999 order by urut asc";
		$query = $this->db->query($sql);
		$data	= $query->result_array();
		for ($i=0; $i<count($data); $i++)
		{
