<?php class Database_model extends CI_Model {
	...
	private $versionMigrate = array(
		'19.02' => array('migrate' => 'migrasi_1902_ke_100', 'nextVersion' => '1.0.0'),
		'1.0.0' => array('migrate' => 'migrasi_100_ke_101', 'nextVersion' => '1.0.1'),
		'1.0.2.1' => array('migrate' => 'migrasi_1021_ke_103', 'nextVersion' => '1.0.3'),
		'1.0.3' => array('migrate' => 'migrasi_103_ke_104', 'nextVersion' => '1.0.4'),
	);
	...
	private function reset_setting_aplikasi()
	{
		...
		(10, 'admin_title','Portal Desa Digital','Judul tab browser modul administrasi','',''),
		...
		(17, 'libreoffice_path','','Path tempat instal libreoffice di server PDD','','')
		";
		$this->db->query($query);
	}
	...
	public function migrasi_db_cri()
	{
		...
		*/
		...
		{
			$this->migrasi_1902_ke_100();
			$this->migrasi_100_ke_101();
			$this->migrasi_1021_ke_103();
			$this->migrasi_103_ke_104_beta();
		}

		private function migrasi_1902_ke_100()
		{
			// tambah slug pada artikel
			if ($this->db->table_exists('artikel') )
			{
				$this->db->query("ALTER TABLE `artikel` ADD `slug` VARCHAR(100) NOT NULL AFTER `judul`");
				// update semua data artikel slug (ambil dari judul)
				$this->db->query("UPDATE artikel SET slug = LOWER( REPLACE( judul, ' ', '-' ) )");
			}

			// tambah kategori artikel transparansi dana desa
			if ($this->db->table_exists('kategori') )
			{
				$this->db->query("ALTER TABLE `kategori` ADD `kategori_slug` VARCHAR(100) NOT NULL AFTER `kategori`");
				
				$this->db->query("UPDATE kategori SET kategori_slug = LOWER( REPLACE( kategori, ' ', '-' ) )");
				if( $this->db->get_where('kategori', array( 'id' => 25 ) )->num_rows() == 0 )
				{
					$this->db->query("INSERT INTO `kategori` (`id`, `kategori`, `kategori_slug`, `tipe`, `urut`, `enabled`, `parrent`) VALUES
					(25, 'Transparansi Dana Desa', 'transparansi-dana-desa', 1, 7, 1, 0)");
				}
			}

			// tambah field petugas pada analisis master
			if ($this->db->table_exists('analisis_master') )
			{
				$this->db->query("ALTER TABLE `analisis_master` ADD `petugas` VARCHAR(100) NOT NULL AFTER `subjek_tipe`");
			}

			// tambah field petugas pada analisis master
			if ($this->db->table_exists('config') )
			{
				$this->db->query("ALTER TABLE `config` ADD `tentang` TEXT");
			}

			if ($this->db->table_exists('setting_aplikasi') )
			{
				$this->load->model('first_m');
				$datadesa = $this->first_m->get_data();
				$namadesa = $this->setting->sebutan_desa.' '.unpenetration($datadesa['nama_desa']);

				if( $this->db->get_where('setting_aplikasi', array( 'id' => 30 ) )->num_rows() == 0 )
				{
					$query = "INSERT INTO `setting_aplikasi` (`id`, `key`, `value`, `keterangan`, `jenis`, `kategori`) VALUES
					(30, 'copyright_desa', '&copy; 2019. ".ucwords( $namadesa ) . "', 'Copyright Web Desa', '', 'web');
					";
					$this->db->query($query);
				}
			}
			...
			// insert PKK, BPD dan LPMD ke user_group
			if ($this->db->table_exists('user_grup') )
			{
				if( $this->db->get_where('user_grup', array( 'id' => 22 ) )->num_rows() == 0 ) {
					$this->db->query("INSERT INTO `user_grup` (`id`, `nama`) VALUES (22, 'PKK')");
				}
				if( $this->db->get_where('user_grup', array( 'id' => 33 ) )->num_rows() == 0 ) {
					$this->db->query("INSERT INTO `user_grup` (`id`, `nama`) VALUES (33, 'BPD')");
				}
				if( $this->db->get_where('user_grup', array( 'id' => 44 ) )->num_rows() == 0 ) {
					$this->db->query("INSERT INTO `user_grup` (`id`, `nama`) VALUES (44, 'LPMD')");
				}
			}
			...
			$this->db->query("ALTER TABLE `agenda` ADD CONSTRAINT `agenda_fk1` FOREIGN KEY (`id_artikel`) REFERENCES `artikel` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION");

			// create tabel apbdes
			if (!$this->db->table_exists('apbdes') )
			{
				$query = "CREATE TABLE `apbdes` (
			...
			$this->db->query("ALTER TABLE `apbdes` ADD CONSTRAINT `FK_apbdes` FOREIGN KEY (`id_kategori`) REFERENCES `apbdes_kategori` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION");

			// create tabel apbdes_kategori
			if (!$this->db->table_exists('apbdes_kategori') )
			{
				$query = "CREATE TABLE `apbdes_kategori` (
			...
			// tmabah modul (menu admin) ke tb setting_modul
			if ($this->db->table_exists('setting_modul') )
			{
				if( $this->db->get_where('setting_modul', array( 'id' => 201 ) )->num_rows() == 0 ) {
					$query = "INSERT INTO `setting_modul` (`id`, `modul`, `url`, `aktif`, `ikon`, `urut`, `level`, `hidden`, `ikon_kecil`, `parent`) VALUES
			...
			// create tb polling
			if (!$this->db->table_exists('polling') )
			{
				$query = "CREATE TABLE `polling` (
				`id` int(11) NOT NULL AUTO_INCREMENT,
				...
				{
					$query = "INSERT INTO `polling` (`id`, `pertanyaan`, `status`) VALUES
					(NULL, 'Bagaimana Pelayanan dari Pemerintah Desa ?', '1');
					";
					$this->db->query($query);
				}
			}

			// create tb poll_pilihan
			if (!$this->db->table_exists('poll_pilihan') )
			{
				$query = "CREATE TABLE `poll_pilihan` (
				`id` int(11) NOT NULL AUTO_INCREMENT,
				...
				$query = "INSERT INTO `poll_pilihan` (`id`, `id_poll`, `nama`, `status`) VALUES
				(NULL, 1, 'Tidak Puas', '1'),
				(NULL, 1, 'Puas', '1'),
				(NULL, 1, 'Sangat Puas', '1');";
				$this->db->query($query);
			}
			$this->db->query("ALTER TABLE `poll_pilihan` ADD CONSTRAINT `poll_pilihan_ibfk_1` FOREIGN KEY (`id_poll`) REFERENCES `polling` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION");

			// create tb poll_vote
			if (!$this->db->table_exists('poll_vote') )
			{
				$query = "CREATE TABLE `poll_vote` (
				`id` int(11) NOT NULL AUTO_INCREMENT,
				`id_poll` int(11) NOT NULL,
			...
			$this->db->query("ALTER TABLE `poll_vote` ADD CONSTRAINT `poll_vote_ibfk_1` FOREIGN KEY (`id_poll`) REFERENCES `polling` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION");
			$this->db->query("ALTER TABLE `poll_vote` ADD CONSTRAINT `poll_vote_ibfk_2` FOREIGN KEY (`id_pil`) REFERENCES `poll_pilihan` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION");

			// tambah widget polling
			if ($this->db->table_exists('widget') && $this->db->table_exists('polling') )
			{
				if( $this->db->get_where('widget', array( 'isi' => 'polling.php' ) )->num_rows() == 0 )
				{
					$query = "INSERT INTO `widget` (`isi`, `enabled`, `judul`, `jenis_widget`, `urut`, `form_admin`, `setting`) VALUES
					('polling.php', 1, 'Jajak Pendapat', 2, 3, '', '');
					";
					$this->db->query($query);
				}
			}

			// create tabel inventaris elektronik
			if (!$this->db->table_exists('inventaris_elektronik') )
			{
				$query = "CREATE TABLE `inventaris_elektronik` (
				`id` int(11) NOT NULL,
			...
			// create tabel mutasi_inventaris_elektronik
			if (!$this->db->table_exists('mutasi_inventaris_elektronik') )
			{
				$query = "CREATE TABLE `mutasi_inventaris_elektronik` (
				`id` int(11) NOT NULL,
				...
				`visible` int(1) NOT NULL DEFAULT '1',
				PRIMARY KEY (id),
				INDEX (id_inventaris_elektronik)
				);
				";
				$this->db->query($query);
			}
			$this->db->query("ALTER TABLE `mutasi_inventaris_elektronik`
				ADD CONSTRAINT `FK_mutasi_inventaris_elektronik` FOREIGN KEY (`id_inventaris_elektronik`) REFERENCES `inventaris_elektronik` (`id`)");

			// OPERASI ALTER
			$this->db->query("RENAME TABLE `inventaris_peralatan` TO `inventaris_mesin`");
			$this->db->query("ALTER TABLE `inventaris_mesin` CHANGE `id_inventaris_peralatan` `id_inventaris_mesin` INT(11) NULL DEFAULT NULL");
			$this->db->query("ALTER TABLE `mutasi_inventaris_mesin` DROP FOREIGN KEY `FK_mutasi_inventaris_peralatan`");
			$this->db->query("ALTER TABLE `mutasi_inventaris_mesin` ADD CONSTRAINT `FK_mutasi_inventaris_mesin` FOREIGN KEY (`id_inventaris_mesin`) REFERENCES `inventaris_mesin` (`id`)");
			$this->db->query("RENAME TABLE `mutasi_inventaris_peralatan` TO `mutasi_inventaris_mesin`");
		}

		private function migrasi_100_ke_101()
		{
			if ($this->db->table_exists('tweb_desa_pamong') )
			{
				$this->db->query("ALTER TABLE `tweb_desa_pamong` ADD `pamong_ttd` TINYINT(1) NULL DEFAULT NULL");
				$this->db->query("ALTER TABLE `tweb_desa_pamong` ADD `pamong_ub` TINYINT(1) NULL DEFAULT NULL");
			}

			if ($this->db->table_exists('tweb_penduduk') )
			{
				$this->db->query("ALTER TABLE `tweb_penduduk` ADD `tag_id_card` VARCHAR(15) NULL DEFAULT NULL");
				$this->db->query("ALTER TABLE `tweb_penduduk` ADD `tabel4_length` VARCHAR(15) NULL DEFAULT NULL");
			}
		}
		
		private function migrasi_1021_ke_103()
		{
			// Tambah table komentar_reply
			if (!$this->db->table_exists('komentar_reply'))
			{
				$this->db->query("CREATE TABLE `komentar_reply` ( `id` INT NOT NULL AUTO_INCREMENT , `admin` INT(1) NOT NULL DEFAULT '1' , `tgl` INT NOT NULL , `pesan` TEXT NOT NULL , `id_komentar` INT NOT NULL , PRIMARY KEY (`id`)) ENGINE = InnoDB;");
			}

			// Tambah tombol media sosial whatsapp
			if ($this->db->table_exists('media_sosial') )
			{
				if( $this->db->get_where('media_sosial', array( 'id' =>6 ) )->num_rows() == 0 ) {
					$query = "INSERT INTO `media_sosial` (`id`, `gambar`, `link`, `nama`, `enabled`) VALUES
					(6, 'wa.png', '', 'WhatsApp', 1);
					";
					$this->db->query($query);
				}
			...
			// Tambah data pada table kategori
			if ($this->db->table_exists('kategori') )
			{
				if( $this->db->get_where('kategori', array( 'id' => 987 ) )->num_rows() == 0 ) {
					$query = "INSERT INTO `kategori` (`id`, `kategori`, `kategori_slug`, `tipe`, `urut`, `enabled`, `parrent`) VALUES
					(987, 'Sambutan dan Himbauan', 'sambutan-dan-himbauan', 2, 2, 1, 0),
					(999, 'Profil Desa', 'profi-desa', 2, 1, 1, 0),
			...
			// Tambah Kolom pada table komentar
			if ($this->db->table_exists('komentar') )
			{
				$this->db->query("ALTER TABLE `komentar` ADD `judul` VARCHAR(100) NOT NULL AFTER `id_artikel`");
				$this->db->query("ALTER TABLE komentar ADD `meta` longtext AFTER no_hp;");
				$this->db->query("ALTER TABLE `komentar` ADD `jenis` INT(1) NOT NULL AFTER `no_hp`;");
				$this->db->query("ALTER TABLE `komentar` ADD `gambar1` TEXT NULL AFTER `jenis`, ADD `gambar2` TEXT NULL AFTER `gambar1`, ADD `gambar3` TEXT NULL AFTER `gambar2`;");
				$this->db->query("ALTER TABLE `komentar` ADD `status` VARCHAR(255) NULL AFTER `enabled`;");
			}

			// Hapus Kolom pada table komentar
			if ($this->db->table_exists('komentar') )
			{
				$this->db->query("ALTER TABLE `komentar`
				DROP `meta`;");
			}

			// Tambah Kolom pada table tweb_desa_pamong
			if ($this->db->table_exists('tweb_desa_pamong') )
			{
				$this->db->query("ALTER TABLE `tweb_desa_pamong` ADD `tupoksi` TEXT NOT NULL AFTER `jabatan`");	
			}

			// Tambah Kolom pada table artikel
			if ($this->db->table_exists('artikel') )
			{
				$this->db->query("ALTER TABLE `artikel` ADD `link_embed` TEXT NULL DEFAULT NULL AFTER `gambar3`");
				$this->db->query("ALTER TABLE `artikel` ADD `sumber_berita` VARCHAR(100) NULL DEFAULT NULL AFTER `link_embed`");
				$this->db->query("ALTER TABLE `artikel` ADD `link_sumber_berita` VARCHAR(100) NULL DEFAULT NULL AFTER `sumber_berita`");
				$this->db->query("ALTER TABLE `artikel` ADD `hit` INT(1) NOT NULL DEFAULT '0' AFTER `boleh_komentar`");
				$this->db->query("ALTER TABLE `artikel` ADD `by_warga` INT(1) NOT NULL DEFAULT '0' AFTER `hit`");
			}

			// create tabel pembangunan
			if (!$this->db->table_exists('pembangunan') )
			{
				$query = "CREATE TABLE `pembangunan` (
			...
			// create tabel potensi
			if (!$this->db->table_exists('potensi') )
			{
				$query = "CREATE TABLE `potensi` (
			...
			// create tabel sambutan
			if (!$this->db->table_exists('sambutan') )
			{
				$query = "CREATE TABLE `sambutan` (
				`id_sambutan` int(11) NOT NULL AUTO_INCREMENT,
				`pemberi_sambutan` varchar(500) NOT NULL,
				`jabatan_sambutan` varchar(500) NOT NULL,
				`foto_sambutan` varchar(200) NULL DEFAULT NULL,
				`id_artikel` int(11) NOT NULL,
			...
			if ($this->db->table_exists('setting_aplikasi') ) {
			    $this->db->query("ALTER TABLE setting_aplikasi CHANGE COLUMN value value TEXT NULL");
			    $isi = htmlentities('<a href="https://www.portaldesadigital.id" target="_blank">Portal Desa Digital</a> dikembangkan oleh Ayo Kreatif');
			    if( $this->db->get_where('setting_aplikasi', array( 'id' => 33 ) )->num_rows() == 0 ) {
			     $query = "INSERT INTO setting_aplikasi (`id`, `key`, `value`, `keterangan`, `jenis`, `kategori`) VALUES (33, 'footer_aplikasi', '$isi', 'Footer Aplikasi', '', 'development')";
			     $this->db->query($query);
			    }
			}


			// Tambah data pada table setting_modul
			if ($this->db->table_exists('setting_modul') )
			{
				if( $this->db->get_where('setting_modul', array( 'id' => 208 ) )->num_rows() == 0 ) {
					$query = "INSERT INTO setting_modul (id, modul, url, aktif, ikon, urut, level, hidden, ikon_kecil, parent) VALUES 
					(208, 'Pesan', 'kontak/kelola', '1', 'fa-envelope', '4', '4', '0', '', '13');
					";
					$this->db->query($query);
				}
			}

			// Tambah data pada table setting_aplikasi
			if ($this->db->table_exists('setting_aplikasi') )
			{
				if( $this->db->get_where('setting_aplikasi', array( 'id' => 31 ) )->num_rows() == 0 ) {
					$query = "INSERT INTO `setting_aplikasi` (`id`, `key`, `value`, `keterangan`, `jenis`, `kategori`) VALUES
			...
			if ($this->db->table_exists('setting_aplikasi') )
			{
				if( $this->db->get_where('setting_aplikasi', array( 'id' => 34 ) )->num_rows() == 0 ) {
					$query = "INSERT INTO `setting_aplikasi` (`id`, `key`, `value`, `keterangan`, `jenis`, `kategori`) VALUES
					(34, 'email_username', 'emailaktifdesa@gmail.com', 'Login Akun Email', '', 'email.config'),
					(35, 'email_port', '465', 'Port SMTP', 'int', 'email.config'),
					(36, 'email_server', 'smtp.gmail.com', 'Server Layanan Email', NULL, 'email.config'),
					(37, 'email_password', 'password email desa', 'Kata Sandi Email', NULL, 'email.config'),
					(38, 'email_sender', 'Portal Desa Digital', 'Nama Pengirim Email', NULL, 'email.config');
					";
					$this->db->query($query);
				}
			}

			// create tabel tweb_kontak
			if (!$this->db->table_exists('tweb_kontak') )
			{
			...
			//ganti judul tabel media_sosial
			if ($this->db->table_exists('media_sosial') )
			{
				$this->db->query("ALTER TABLE `media_sosial` CHANGE COLUMN `gambar` `icon` TEXT NOT NULL AFTER `id`;");
				
				$this->db->query($query);
			}
			
			//icon sosial media dari gambar(.png) menjadi fontawesome
			if ($this->db->table_exists('media_sosial') )
			{
				// ganti isi tabel media_sosial
				$this->db->query("UPDATE media_sosial SET icon = 'fa fa-facebook'  WHERE id = '1'");
				$this->db->query("UPDATE media_sosial SET icon = 'fa fa-twitter' WHERE id = '2'");
				$this->db->query("UPDATE media_sosial SET icon = 'fa fa-google' WHERE id = '3'");
				$this->db->query("UPDATE media_sosial SET icon = 'fa fa-youtube'  WHERE id = '4'");
				$this->db->query("UPDATE media_sosial SET icon = 'fa fa-instagram' WHERE id = '5'");
				$this->db->query("UPDATE media_sosial SET icon = 'fa fa-whatsapp'  WHERE id = '6'");
				$this->db->query($query);
			}
		}

		private function migrasi_103_ke_104()
		{
			if ($this->db->table_exists('pembangunan') )
			{
				$this->db->query("ALTER TABLE `pembangunan` ADD `bidang_pembangunan` VARCHAR(225) NOT NULL AFTER `id_pembangunan`");
				$this->db->query("ALTER TABLE `pembangunan` ADD `sub_bidang_pembangunan` VARCHAR(225) NOT NULL AFTER `bidang_pembangunan`");
				$this->db->query("ALTER TABLE `pembangunan` ADD `nama_kegiatan_pembangunan` VARCHAR(225) NOT NULL AFTER `sub_bidang_pembangunan`");
				$this->db->query("ALTER TABLE `pembangunan` ADD `volume_kegiatan_pembangunan` VARCHAR(100) NOT NULL AFTER `nama_kegiatan_pembangunan`");
				$this->db->query("ALTER TABLE `pembangunan` ADD `sumber_dana_pembangunan` VARCHAR(225) NOT NULL AFTER `nilai_pembangunan`");
				$this->db->query("ALTER TABLE `pembangunan` ADD `waktu_pelaksanaan_pembangunan` VARCHAR(100) NOT NULL AFTER `sumber_dana_pembangunan`");
			}

			if ($this->db->table_exists('area'))
			{
				$this->db->query("ALTER TABLE `area` CHANGE `path` `path` TEXT CHARACTER SET latin1 COLLATE latin1_swedish_ci NULL");
				$this->db->query("ALTER TABLE `area` CHANGE `foto` `foto` VARCHAR(250) CHARACTER SET latin1 COLLATE latin1_swedish_ci DEFAULT NULL");
				$this->db->query("ALTER TABLE `area` CHANGE `id_cluster` `id_cluster` INT(11) DEFAULT NULL");
				$this->db->query("ALTER TABLE `area` CHANGE `desk` `desk` TEXT CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL");
			}

			if ($this->db->table_exists('dokumen'))
			{
				$this->db->query("ALTER TABLE `dokumen` CHANGE `nama` `nama` VARCHAR(250) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL");
			}

			if ($this->db->table_exists('garis'))
			{
				$this->db->query("ALTER TABLE `garis` CHANGE `path` `path` TEXT CHARACTER SET latin1 COLLATE latin1_swedish_ci NULL");
			}

			if ($this->db->table_exists('lokasi'))
			{
				$this->db->query("ALTER TABLE `lokasi` CHANGE `lat` `lat` VARCHAR(30) CHARACTER SET latin1 COLLATE latin1_swedish_ci NULL");
				$this->db->query("ALTER TABLE `lokasi` CHANGE `lng` `lng` VARCHAR(30) CHARACTER SET latin1 COLLATE latin1_swedish_ci NULL");
				$this->db->query("ALTER TABLE `lokasi` CHANGE `foto` `foto` VARCHAR(250) CHARACTER SET latin1 COLLATE latin1_swedish_ci DEFAULT NULL");
				$this->db->query("ALTER TABLE `lokasi` CHANGE `id_cluster` `id_cluster` INT(11) DEFAULT NULL");
				$this->db->query("ALTER TABLE `lokasi` CHANGE `desk` `desk` TEXT CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL");
			}				

			if ($this->db->table_exists('polygon'))
			{
				$this->db->query("ALTER TABLE `polygon` CHANGE `tipe` `tipe` INT(4) NOT NULL DEFAULT '0'");
			}

			if ($this->db->table_exists('line'))
			{
				$this->db->query("ALTER TABLE `line` CHANGE `tipe` `tipe` INT(4) NOT NULL DEFAULT '0'");
			}

			if ($this->db->table_exists('point'))
			{
				$this->db->query("ALTER TABLE `point` CHANGE `tipe` `tipe` INT(4) NOT NULL DEFAULT '0'");
			}

			if ($this->db->table_exists('tweb_penduduk_umur'))
			{
				$this->db->query("UPDATE tweb_penduduk_umur SET nama='TUA', dari='31', sampai='70', status='0' WHERE id='4'");
			}

			if ($this->db->table_exists('tweb_penduduk_umur'))
			{
				$query = "INSERT INTO `tweb_penduduk_umur` (`id`, `nama`, `dari`, `sampai`, `status`) VALUES
				(5, 'MANULA', 71, 120, 0);";
				$this->db->query($query);
			}
			...
			{
				$query = "INSERT INTO `widget` (`id`, `isi`, `enabled`, `judul`, `jenis_widget`, `urut`, `form_admin`, `setting`) VALUES (20, 'popular_artikel.php', '1', 'Populer Artikel', '1', '5', '', '');";
				$this->db->query($query);
			}

			if ($this->db->table_exists('setting_modul'))
			{
				$query = "INSERT INTO `setting_modul` (`id`, `modul`, `url`, `aktif`, `ikon`, `urut`, `level`, `hidden`, `ikon_kecil`, `parent`) VALUES ('209', 'Lapak', 'lapak_sid', '1', 'fa-shopping-cart', '9', '2', '0', 'fa-shopping-cart', '13');";
				$this->db->query($query);
			}

			if (!$this->db->table_exists('lapak_barang'))
			{
				$query="CREATE TABLE `lapak_barang` (
			...
			if (!$this->db->table_exists('lapak_gallery'))
			{
				$query="CREATE TABLE `lapak_gallery` (
				  `id` int(11) NOT NULL AUTO_INCREMENT,
			...
			if (!$this->db->table_exists('lapak_toko'))
			{
				$query="CREATE TABLE `lapak_toko` (
				  `id` int(11) NOT NULL AUTO_INCREMENT,
			...
			// Creat Table IDM //
			if (!$this->db->table_exists('idm'))
			{
				$query="CREATE TABLE `idm` (
				`id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
				`tahun` year(4) NOT NULL,
			...
			if ($this->db->table_exists('setting_modul'))
			{
				$query = "INSERT INTO `setting_modul` (`id`, `modul`, `url`, `aktif`, `ikon`, `urut`, `level`, `hidden`, `ikon_kecil`, `parent`) VALUES (210, 'Indeks Desa Membangun', 'idm_sid', '1', 'fa-pie-chart', '3', '3', '0', 'fa-pie-chart', '3');";
				$this->db->query($query);
			}
			...
			if ($this->db->table_exists('area'))
			{
				$this->db->query("ALTER TABLE `area` ADD `warna` TEXT NULL AFTER `enabled`;");
			}
			// Tambah Kolom pada table artikel
			if ($this->db->table_exists('artikel') )
			{
				$this->db->query("ALTER TABLE `artikel` ADD `caption` VARCHAR(100) NULL AFTER `gambar`");
				$this->db->query("ALTER TABLE `artikel` ADD `caption1` VARCHAR(100) NULL AFTER `gambar1`");
				$this->db->query("ALTER TABLE `artikel` ADD `caption2` VARCHAR(100) NULL AFTER `gambar2`");
				$this->db->query("ALTER TABLE `artikel` ADD `caption3` VARCHAR(100) NULL AFTER `gambar3`");
			}

			if (!$this->db->table_exists('tweb_aset'))
			{
				{
					$query = "CREATE TABLE `tweb_aset` (
					...
					$query = "INSERT INTO `tweb_aset` (`id_aset`, `golongan`, `bidang`, `kelompok`, `sub_kelompok`, `sub_sub_kelompok`, `nama`) VALUES
					...
					(2659, '2', '06', '03', '18', '001', 'PENDINGIN UDARA'),
					(2660, '2', '06', '03', '18', '002', 'PENDINGIN AIR'),
					(2661, '2', '06', '03', '18', '003', 'PENDINGIN MINYAK'),
					(2662, '2', '06', '03', '18', '004', 'PENDINGIN GAS'),
					(2663, '2', '06', '03', '18', '999', 'PERALATAN LAINNYA'),
					...
					(3617, '4', '02', '04', '01', '001', 'BANGUNAN WASUK PENGAMAN SUNGAI/PANTAI'),
