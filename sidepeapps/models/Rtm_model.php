<?php class Rtm_model extends CI_Model {
	...
	{
		if (isset($_SESSION['sex']) AND ($_SESSION['sex'] == 1 || $_SESSION['sex'] == 2))
		{
			$kh = $_SESSION['sex'];
			$jenis_sql = " AND sex = $kh";
			return $jenis_sql;
		}
	}
	...
	public function wilayah()
	{
		...
		(SELECT COUNT(rm.id) FROM tweb_rtm rm inner join tweb_penduduk p ON rm.nik_kepala = p.id WHERE p.kk_level = 1 and status_dasar = 1) AS total_rumah_tangga,
		(SELECT COUNT(rm.id) FROM tweb_rtm rm inner join tweb_penduduk p ON rm.nik_kepala = p.id  WHERE p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE dusun = u.dusun) AND p.kk_level = 1 and status_dasar = 1) AS jumlah_rumah_tangga
