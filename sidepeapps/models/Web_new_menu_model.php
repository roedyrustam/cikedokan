...
class Web_new_menu_model extends CI_Model {
	...
	public function list_parent_menu($tip=1)
	{
		$sql = "SELECT * FROM menu WHERE parrent = 1 AND tipe = $tip ORDER BY urut";
	...
	public function list_statis()
	{
		// '999' adalah id_kategori untuk artikel statis
		$sql = "SELECT a.id, a.judul, k.kategori FROM artikel a
		LEFT JOIN kategori k ON k.id=a.id_kategori
	...
	{
		$sql = "SELECT * FROM menu_kategori ORDER BY id ASC";
		$query = $this->db->query($sql);
		$data  = $query->result_array();
		return $data;
	}
