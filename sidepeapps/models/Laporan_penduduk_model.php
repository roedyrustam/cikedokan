<?php class Laporan_penduduk_model extends CI_Model {
	...
	public function judul_statistik($lap)
	{
		...
		switch ("$lap")
		{
			...
			case "rumah_tangga": return "Rumah Tangga"; break;
	...
	public function list_data($lap=0, $o=0)
	{
		...
		switch ("$lap")
		{
			...
			case 'rumah_tangga': $sql   = "SELECT u.*, u.dusun as nama,





		(SELECT COUNT(rm.id) FROM tweb_rtm rm inner join tweb_penduduk p ON rm.nik_kepala = p.id  WHERE p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE dusun = u.dusun) AND p.kk_level = 1 and status_dasar = 1) AS jumlah,


		(SELECT COUNT(rm.id) FROM tweb_rtm rm inner join tweb_penduduk p ON rm.nik_kepala = p.id  WHERE p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE dusun = u.dusun) AND p.kk_level = 1 and status_dasar = 1 AND p.sex = 1) AS laki,

		(SELECT COUNT(rm.id) FROM tweb_rtm rm inner join tweb_penduduk p ON rm.nik_kepala = p.id  WHERE p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE dusun = u.dusun) AND p.kk_level = 1 and status_dasar = 1 AND p.sex = 2) AS perempuan
		...
		elseif ("$lap" == 'rumah_tangga')
		{
			$sql3 = "SELECT (SELECT COUNT(rm.id) FROM tweb_rtm rm inner join tweb_penduduk p ON rm.nik_kepala = p.id  WHERE p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE dusun = u.dusun) AND p.kk_level = 1 and status_dasar = 1) AS jumlah,
		...
		$data[$i]['laki'] = $total['laki'];
			...
			for ($i=0; $i<count($data); $i++)
		{
			$data[$i]['persen'] = $data[$i]['jumlah']/$total['jumlah']*100;
			$data[$i]['persen'] = number_format((float)$data[$i]['persen'], 2, '.', '');
			$data[$i]['persen'] = $data[$i]['persen']."%";

			$data[$i]['persen1'] = $data[$i]['laki']/$total['jumlah']*100;
			$data[$i]['persen1'] = number_format((float)$data[$i]['persen1'], 2, '.', '');
			$data[$i]['persen1'] = $data[$i]['persen1']."%";

			$data[$i]['persen2'] = $data[$i]['perempuan']/$total['jumlah']*100;
			$data[$i]['persen2'] = number_format((float)$data[$i]['persen2'], 2, '.', '');
			$data[$i]['persen2'] = $data[$i]['persen2']."%";
		}
		}
