...
class Inventaris_mesin_model extends CI_Model
{

	protected $table = 'inventaris_mesin';
	protected $table_mutasi = 'mutasi_inventaris_mesin';
	...
	{
		$this->db->select('mutasi_inventaris_mesin.id as id,mutasi_inventaris_mesin.*,  inventaris_mesin.nama_barang, inventaris_mesin.kode_barang, inventaris_mesin.tahun_pengadaan');
		$this->db->from($this->table_mutasi);
		$this->db->where($this->table_mutasi.'.visible', 1);
		$this->db->join($this->table, $this->table.'.id = '.$this->table_mutasi.'.id_inventaris_mesin', 'left');
		$data = $this->db->get()->result();
		return $data;
	}
	...
	public function add_mutasi($data)
	{
		...
		$id = $this->db->insert_id();
		$this->db->update($this->table, array('status' => 1), array('id' => $data['id_inventaris_mesin']));
	...
	{
		$this->db->select('mutasi_inventaris_mesin.id as id,mutasi_inventaris_mesin.*,  inventaris_mesin.nama_barang, inventaris_mesin.kode_barang, inventaris_mesin.tahun_pengadaan, inventaris_mesin.register');
		$this->db->from($this->table_mutasi);
