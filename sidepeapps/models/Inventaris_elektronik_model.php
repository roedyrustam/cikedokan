...
class Inventaris_elektronik_model extends CI_Model
{

	protected $table = 'inventaris_elektronik';
	protected $table_mutasi = 'mutasi_inventaris_elektronik';
	...
	function last_reg()
	{
		$last_data = $this->db->select("MAX(SUBSTRING(register,-6)) as kode")->limit(1)->get($this->table)->row();

		$data = $this->db->select("MAX(SUBSTRING(register,-6)) as kode, register")->where('SUBSTRING(register,-6)', $last_data->kode)->get($this->table)->row();
		return $data;
	}
	...
	{
		$this->db->select('mutasi_inventaris_elektronik.id as id,mutasi_inventaris_elektronik.*,  inventaris_elektronik.nama_barang, inventaris_elektronik.kode_barang, inventaris_elektronik.tahun_pengadaan');
		$this->db->from($this->table_mutasi);
		$this->db->where($this->table_mutasi.'.visible', 1);
		$this->db->join($this->table, $this->table.'.id = '.$this->table_mutasi.'.id_inventaris_elektronik', 'left');
		$data = $this->db->get()->result();
		return $data;
	}
	...
	public function add_mutasi($data)
	{
		...
		$id = $this->db->insert_id();
		$this->db->update($this->table, array('status' => 1), array('id' => $data['id_inventaris_elektronik']));
	...
	{
		$this->db->select('mutasi_inventaris_elektronik.id as id,mutasi_inventaris_elektronik.*,  inventaris_elektronik.nama_barang, inventaris_elektronik.kode_barang, inventaris_elektronik.tahun_pengadaan, inventaris_elektronik.register');
		$this->db->from($this->table_mutasi);
	...
	{
		if(is_null($id)) $id = $this->input->post('id');
