<div class="content-wrapper">
	<section class="content-header">
		<div class="title-header">Daftar Inventaris Mesin</div>
		<ol class="breadcrumb">
			<li><a href="<?= site_url('hom_sid')?>"><i class="fa fa-home"></i> Home</a></li>
			<li class="active">Daftar Inventaris Mesin</li>
		</ol>
	</section>
				...
				<div class="col-md-9">
					<div class="box box-info" style="border-radius:10px; border-top:0;padding:10px">
						<div class="box-header with-border">
							<a href="<?= base_url($this->controller.'/form')?>" class="btn btn-uppercase btn-radius btn-sm btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" title="Tambah Data Baru" style="background-color:#00ca6d">Tambah Data</a>
							<a href="#" class="btn btn-uppercase btn-radius bg-purple btn-sm btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" title="Cetak Data" data-remote="false" data-toggle="modal" data-target="#cetakBox" data-title="Cetak Inventaris"></i>Cetak Data</a>
																	...
																	<?php else: ?>
																		...
																		<td nowrap>
																			<?php if ($data->status == "0"): ?>
																				<a href="<?= base_url($this->controller.'/form_mutasi/'.$data->id); ?>" title="Mutasi Data" class="btn bg-olive btn-radius btn-sm"><i class="fa fa-external-link-square"></i></a>
																			<?php endif; ?>
																			<a href="<?= base_url($this->controller.'/view/'.$data->id); ?>" title="Lihat Data" class="btn btn-info btn-radius-sm btn-sm"><i class="fa fa-eye"></i></a>
																			<a href="<?= base_url($this->controller.'/edit/'.$data->id); ?>" title="Edit Data"  class="btn bg-orange btn-radius-sm btn-sm"><i class="fa fa-edit"></i> </a>
																			<a href="#" data-href="<?= site_url("api_$this->controller/delete/$data->id")?>" class="btn bg-maroon btn-radius-sm btn-sm"  title="Hapus" data-toggle="modal" data-target="#confirm-delete"><i class="fa fa-trash-o"></i></a>
																		</td>
...
<script>
	...
	{
		var link = '<?= site_url("$this->controller/cetak"); ?>'+ '/' + $('#tahun_pdf').val() + '/' + $('#penandatangan_pdf').val();
		window.open(link, '_blank');
	});
	$("#form_download").click(function(event)
	{
		var link = '<?= site_url("$this->controller/download"); ?>'+ '/' + $('#tahun').val() + '/' + $('#penandatangan').val();
		window.open(link, '_blank');
	});
</script>