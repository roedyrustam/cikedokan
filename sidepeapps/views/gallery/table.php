...
<div class="content-wrapper">
	<section class="content-header">
		<div class="title-header">Daftar Album</div>
		<ol class="breadcrumb">
				...
				<div class="col-md-12">
					<div class="box box-info">
            			<div class="box-header with-border">
							<a href="<?= site_url("gallery/form")?>" class="btn btn-radius btn-uppercase btn-success btn-sm btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block"  title="Tambah Artikel">Tambah Album</a>
							<a href="#confirm-delete" title="Hapus Data" onclick="deleteAllBox('mainform', '<?= site_url("gallery/delete_all/$p/$o")?>')" class="btn btn-radius btn-uppercase btn-danger btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block hapus-terpilih">Hapus Data Terpilih</a>
						</div>
						<div class="box-body">
							<div class="row">
								<div class="col-sm-12">
													...
													<div class="box-tools">
														<div class="input-group input-group-sm pull-right">
															...
															<div class="input-group-btn">
																<button type="submit" class="btn btn-default" onclick="$('#'+'mainform').attr('action', '<?= site_url("gallery/search")?>');$('#'+'mainform').submit();" style="border-top-left-radius:0;border-bottom-left-radius:0"><i class="fa fa-search"></i></button>
															</div>
														</div>
													</div>
												</div>
															...
															<tbody>
																<?php foreach ($main as $data): ?>
																	<tr>
																		...
																		<td><?=$data['no']?></td>
																		<td nowrap>
																			<a href="<?= site_url("gallery/sub_gallery/$data[id]")?>" class="btn bg-purple btn-radius-sm btn-sm"  title="Rincian Album"><i class="fa fa-bars"></i></a>
																			<a href="<?= site_url("gallery/form/$p/$o/$data[id]")?>" class="btn btn-warning btn-radius-sm btn-sm"  title="Ubah"><i class="fa fa-edit"></i></a>
																			<?php if ($data['slider'] == '1'): ?>
																				<a href="<?= site_url("gallery/slider_off/".$data['id'])?>" class="btn bg-maroon btn-radius-sm btn-sm" title="Keluarkan Dari Slider"><i class="fa fa-play"></i></a>
																			<?php else: ?>
																				<a href="<?= site_url("gallery/slider_on/".$data['id'])?>" class="btn bg-maroon btn-radius-sm btn-sm"  title="Tampilkan Di Slider"><i class="fa fa-eject"></i></a>
																			<?php endif; ?>
																			<?php if ($_SESSION['grup']<4): ?>
									                                        <?php if ($data['enabled'] == '2'): ?>
									                                          <a href="<?= site_url("gallery/gallery_lock/".$data['id'])?>" class="btn bg-navy btn-radius-sm btn-sm"  title="Aktifkan Album"><i class="fa fa-lock"></i></a>
									                                        <?php elseif ($data['enabled'] == '1'): ?>
									                                          <a href="<?= site_url("gallery/gallery_unlock/".$data['id'])?>" class="btn bg-navy btn-radius-sm btn-sm"  title="Non Aktifkan Album"><i class="fa fa-unlock"></i></a>
                                        									<?php endif ?>
																				<a href="#" data-href="<?= site_url("gallery/delete/$p/$o/$data[id]")?>" class="btn bg-maroon btn-radius-sm btn-sm"  title="Hapus" data-toggle="modal" data-target="#confirm-delete"><i class="fa fa-trash-o"></i></a>
                                     										<?php endif; ?>
										                                    </td>
										                                    <td width="60%">
