						...
						<div class="box-body">
								...
								<div class="col-sm-4" class="">
									...
									<select name="id_kategori" class="select2 form-control input-sm" required>
										...
										<?php foreach( $kategori_anggaran as $kat ){ ?>
											<optgroup label="<?php echo $kat['kategori']; ?>">
												<?php
												$sub_kategori = $this->kategori_anggaran->list_sub_kategori($kat['id']);
												foreach( $sub_kategori as $sub ){
													?>
													<option value="<?php echo $sub['id']; ?>" <?php echo $sub['id']==$id_kat || $sub['id']==$id_kat ? 'selected' : '' ?>><?php echo $sub['kategori']; ?></option>
												<?php } ?>
											</optgroup>
										<?php } ?>
									</select>
							...
							<div class="form-group">
								...
								<div class="col-sm-2">
									...
									<select onchange="return filter(this.value)" required name="tahun" class="form-control input-sm" <?php echo $aksi == 'update' ? 'disabled' : '' ?>>
								...
								<div class="col-sm-2">
									<select name="tahap" class="form-control input-sm" required <?php echo ( $type==2 ) ? 'disabled' : '' ?>>
										<option value="">-- Pilih Tahap --</option>
										<option value="Tahap 1" <?php echo ($anggaran['tahap']=='Tahap 1') ? 'selected' : '' ?>>Tahap 1</option>
										<option value="Tahap 2" <?php echo ($anggaran['tahap']=='Tahap 2') ? 'selected' : '' ?>>Tahap 2</option>
										<option value="Tahap 3" <?php echo ($anggaran['tahap']=='Tahap 3') ? 'selected' : '' ?>>Tahap 3</option>
									</select>

									<?php if( $type==2 ){ ?>
										<input name="tahap" class="form-control input-sm" type="hidden" value="<?=$anggaran['tahap']?>" />
									<?php } ?>
								</div>
							</div><hr />
...
<script type="text/javascript">
	function filter(tahun) {
		window.location.href = "<?php echo site_url('anggaran/tambah?type='.$type.'&tahun=') ?>" + tahun;
	}
</script>
