...
<div class="content-wrapper">
	<section class="content-header">
		<div class="title-header"><?= $kat_nama?></div>
		<ol class="breadcrumb">
	...
	<section class="content" id="maincontent">
		<form id="mainform" name="mainform" action="" method="post">
			<div class="row">
				<div class="<?php if ($this->modul_ini <> 15): ?>col-md-12<?php else: ?>col-md-12<?php endif; ?>">
					<div class="box box-info">
            			<div class="box-header with-border">
							<a href="<?= site_url("{$this->controller}/form/$kat")?>" class="btn btn-radius btn-uppercase btn-success btn-sm btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block"  title="Tambah Menu Baru">Tambah <?= $kat_nama?> Baru</a>
						<?php if ($_SESSION['grup']<4): ?>
							<a href="#confirm-delete" title="Hapus Data" onclick="deleteAllBox('mainform', '<?= site_url("{$this->controller}/delete_all/$kat/$p/$o")?>')" class="btn btn-radius btn-uppercase btn-danger btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block hapus-terpilih">Hapus Data Terpilih</a>
						<?php endif; ?>
							<a href="<?= site_url("{$this->controller}/dialog_cetak/$kat")?>" class="btn btn-radius btn-uppercase bg-purple btn-sm btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" title="Cetak Dokumen" data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Cetak Laporan">Cetak</a>
							<a href="<?= site_url("{$this->controller}/dialog_excel/$kat")?>" class="btn btn-radius btn-uppercase bg-navy btn-sm btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" title="Unduh Dokumen" data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Unduh Laporan">Unduh</a>
						</div>
						<div class="box-body">
							<div class="row">
								<div class="col-sm-12">
													...
													<div class="box-tools">
														<div class="input-group input-group-sm pull-right">
															...
															<div class="input-group-btn">
																<button type="submit" class="btn btn-default" onclick="$('#'+'mainform').attr('action', '<?= site_url("{$this->controller}/search")?>');$('#'+'mainform').submit();" style="border-top-left-radius:0;border-bottom-left-radius:0"><i class="fa fa-search"></i></button>
															</div>
														</div>
													</div>
												</div>
													...
													<div class="table-responsive">
														<table class="table table-bordered table-striped dataTable table-hover">
															<thead class="bg-gray disabled color-palette">
																...
																<?php endif; ?>
								                                <?php if ($o==4): ?>
								                                    <th class="text-center" nowrap><a href="<?= site_url("{$this->controller}/index/$kat/$p/3")?>">Aktif <i class='fa fa-sort-asc fa-sm'></i></a></th>
								                                <?php elseif ($o==3): ?>
								                                    <th class="text-center" nowrap><a href="<?= site_url("{$this->controller}/index/$kat/$p/4")?>">Aktif <i class='fa fa-sort-desc fa-sm'></i></a></th>
								                                <?php else: ?>
								                                    <th class="text-center" nowrap><a href="<?= site_url("{$this->controller}/index/$kat/$p/3")?>">Aktif <i class='fa fa-sort fa-sm'></i></a></th>
								                                <?php endif; ?>
																<?php if ($o==6): ?>
																...
																<?php foreach ($main as $data): ?>
																	<tr>
																		...
																		<td class="text-center" nowrap>
																			<a href="<?= site_url("{$this->controller}/form/$kat/$p/$o/$data[id]")?>" class="btn btn-warning btn-radius-sm btn-sm"  title="Ubah"><i class="fa fa-edit"></i></a>
																			<?php if ($data['enabled'] == '2'): ?>
																				<a href="<?= site_url($this->controller.'/dokumen_lock/'.$kat.'/'.$data['id'])?>" class="btn bg-navy btn-radius-sm btn-sm"  title="Aktifkan"><i class="fa fa-lock">&nbsp;</i></a>
																			<?php elseif ($data['enabled'] == '1'): ?>
																				<a href="<?= site_url($this->controller.'/dokumen_unlock/'.$kat.'/'.$data['id'])?>" class="btn bg-navy btn-radius-sm btn-sm"  title="Non Aktifkan"><i class="fa fa-unlock"></i></a>
                                      										<?php endif ?>
																			<a href="<?= base_url().LOKASI_DOKUMEN.underscore($data['satuan'])?>" class="btn bg-purple btn-radius-sm btn-sm"  title="Unduh"><i class="fa fa-download"></i></a>
																			<a href="#" data-href="<?= site_url("{$this->controller}/delete/$kat/$p/$o/$data[id]")?>" class="btn bg-maroon btn-radius-sm btn-sm"  title="Hapus" data-toggle="modal" data-target="#confirm-delete"><i class="fa fa-trash-o"></i></a>
																	  	</td>
