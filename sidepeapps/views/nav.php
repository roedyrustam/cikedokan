<aside class="main-sidebar">
	<section class="sidebar">
		<div class="user-panel">
			<div class="image">
				<img src="<?= LogoDesa($desa['logo']);?>" class="img-circle img-center" alt="User Image">
			</div>
			<div class="info">
		...
		<ul class="sidebar-menu" data-widget="tree">
			<?php foreach ($modul AS $mod): ?>
				<li class="<?php if (count($mod['submodul'])>0): ?>treeview<?php endif ?> <?php if ($this->modul_ini==$mod['id']): ?>active<?php endif ?> menu-backend">
					<a href="<?= site_url()?><?=$mod['url']?>">
						<i class="fa <?= $mod['ikon']?> <?php if ($this->modul_ini==$mod['id']): ?><?php endif ?>"></i> <span><?=$mod['modul']?></span>
							...
							<?php foreach ($mod['submodul'] as $submod): ?>
								<li class="<?php if ($act_sub==$submod['id']): ?>active<?php endif ?>">
									<a href="<?= site_url()?><?=$submod['url']?>">
										<i class="fa <?php if ($submod['ikon']!=NULL): ?><?=$submod['ikon']?><?php else: ?> fa-circle-o<?php endif ?> <?php if ($act_sub==$submod['id']): ?><?php endif ?>"></i>
										<?=$submod['modul']?>
									</a>
								</li>
							<?php endforeach; ?>
